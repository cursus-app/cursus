...
const fixture = HARNESS_FIXTURES.find((f) => f.name === 'missing-branch')
if (!fixture) { throw new Error('Fixture "missing-branch" introuvable dans fixtures.config.ts') }

describe.skipIf(!isHarnessConfigured())(`Harness fixture: ${fixture.name}`, () => {
  it(
    'check branch_exists rouge → statut failed',
    async () => {
      const result = await runHarnessAgainst(fixture.repoUrl)

      expect(result.status).toBe<typeof result.status>('failed')
      expect(result.failedChecks).toContain('branch_exists')
    },
    10 * 60 * 1000,
  )
})
